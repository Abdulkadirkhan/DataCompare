...
to have a diff viewer (kdiff3, meld) that can display these. Also, make
sure to turn on word wrap for full effect.
...
def get_filetype(filename):
    """
    Hack to determine the filetype.
    """
    (head, tail) = os.path.split(filename)
    (root, ext) = os.path.splitext(tail)
    lowerext = ext.lower()
    if lowerext in ['.pdf', '.fdf']:
        return "pdf"
    elif lowerext in ['.ps']:
        return "ps"
    else:
...
def fix_ff_problem(sentence):
    ...
    """
    sentence = sentence.replace("?", "ffi")
    sentence = sentence.replace("?", "ffl")
    sentence = sentence.replace("?", "ff")
    return sentence
...
def normalize_anything(filename, fout=sys.stdout):
    ...
    while filetype != "txt":
        if filetype == "pdf":
            ...
            print
            "Error: Don't know how to handle file type '%s'" % (filetype)
            sys.exit(1)
        if temphandle:
            temphandle.close()
...
if __name__ == "__main__":
    ...
    while len(args) > 0:
        ...
        if optcmd in ["-?", "-h", "--help"]:
            ...
            if len(args) < 2:
                print
                "Error: Diff viewer preference requires a string prefix argument"
                sys.exit(1)
            diffViewerPrefix = args[1]
            if len(filter(lambda s: s.startswith(diffViewerPrefix), get_viewer_list())) == 0:
                print
                "Error: No viewer from the list %s starts with '%s'" % (get_viewer_list(), diffViewerPrefix)
                sys.exit(1)
            args = args[2:]
            ...
            if len(args) == 1:
                ...
                print
                "Error: I don't know what to do with more than two files"
                sys.exit(1)

# vim: set ts=4 sw=4 et fileencoding=latin1 list lcs=tab\:>-: